      ...
      <HeadlessListbox value={selectedOption} onChange={onChange}>
        <HeadlessListbox.Button
          className={classNames(
            'text-case-inherit letter-space-inherit flex h-10 w-3/4 items-center justify-between rounded-lg px-4 text-sm font-medium outline-none duration-200 sm:h-12 sm:px-5',
        ...
        <Transition
          ...
          leaveTo="opacity-0"
        >
          <HeadlessListbox.Options className="absolute left-0 z-10 mt-1 grid w-full origin-top-right gap-0.5 overflow-hidden text-ellipsis rounded-lg border border-gray-200 bg-white p-1 shadow-large outline-none dark:border-gray-700 dark:bg-gray-800 xs:p-2">
            {options.map((option) => (
